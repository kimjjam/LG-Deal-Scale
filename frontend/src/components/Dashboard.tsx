...
function DashboardContent({ metrics }: { metrics: CrmDashboard }) {
  ...
  return <>
    ...
    <div className="summary-cards four">
      ...
      <section><span>오늘 마감 할 일</span><strong>{metrics.tasks.due_today}</strong><small>기한 초과 {metrics.tasks.overdue}건 · 진행 중 {metrics.tasks.open}건</small></section>
    </div>
    <div className="metrics-layout dashboard-grid">
      ...
      <section className="metric-block"><div className="section-heading compact"><div><h2>월별 예상 수주</h2><span>진행 중 영업기회의 예상 계약일 기준</span></div></div><div className="metric-table-wrap"><table className="metric-table"><thead><tr><th>월</th><th>건수</th><th>금액</th><th>확률 반영</th></tr></thead><tbody>{metrics.forecast.months.length ? metrics.forecast.months.map((month) => <tr key={month.month}><th>{month.month}</th><td>{month.count}</td><td>{money.format(month.amount)}</td><td>{money.format(month.weighted_amount)}</td></tr>) : <tr><td colSpan={4}>예상 계약일이 등록된 진행 건이 없습니다.</td></tr>}</tbody></table></div><p className="metric-definition">예상 계약일 미등록 {metrics.forecast.missing_close_date}건</p></section>
    </div>
  </>;
}
