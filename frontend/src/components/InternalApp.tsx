...
export default function InternalApp() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("directdesk:session-expired", expire);
  }, []);
  ...
  const currentView =
    view === "staff" && session.role !== "owner" ? "inbox" : view;
  const activeItem =
    visibleItems.find((item) => item.key === currentView) ?? visibleItems[0];
    ...
    <div className="internal-shell">
      <nav className="sidebar" aria-label="주요 메뉴">
        ...
        <a className="public-link" href="/inquiry">
          ...
          <span>
            <strong>공개 문의 페이지</strong>
            <small>고객 화면 열기</small>
          </span>
        </a>
      </nav>
      ...
      <div className="internal-main">
        <header className="topbar">
          <div className="topbar-context">
            <span className="mobile-brand-symbol" aria-hidden="true">
              D
            </span>
            <div>
        ...
        {currentView === "partners" ? (
          <PartnersRegions session={session} />
        ) : null}
        {currentView === "staff" && session.role === "owner" ? (
...
function NavIcon({ name }: { name: View }) {
  const paths: Record<View, string> = {
    ...
    accounts:
      "M4 20V7l8-4 8 4v13 M8 10h2 M14 10h2 M8 14h2 M14 14h2 M10 20v-3h4v3",
    ...
    leads:
      "M12 3l2.2 4.7L19 10l-4.8 2.3L12 17l-2.2-4.7L5 10l4.8-2.3z M5 17l-2 4 M19 17l2 4",
    dashboard: "M4 20V11h3v9z M10.5 20V4h3v16z M17 20v-6h3v6z",
    partners:
      "M4 20V8h7v12 M13 20V4h7v16 M7 11h1 M7 15h1 M16 8h1 M16 12h1 M16 16h1",
    staff:
      "M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8z M4 21a8 8 0 0 1 16 0 M19 8v6 M16 11h6",
  };
  return (
  ...
  return (
    <section className="workspace" aria-labelledby="search-title">
      <div className="commandbar">
        <div>
          <h1 id="search-title">자연어 데이터 검색</h1>
          <p>일상적인 질문으로 CRM 데이터를 안전하게 조회하세요.</p>
        </div>
        <span className="safety-chip">
          <span aria-hidden="true">●</span> 읽기 전용
        </span>
      </div>
      ...
      {result ? (
        <section className="search-results" aria-live="polite">
          <div className="section-heading">
            ...
            <span className="count-chip">{result.rows.length}개 행</span>
          </div>
          {result.rows.length ? (
            <div
              className="data-grid-wrap"
              role="region"
              aria-label="자연어 검색 결과 표, 가로 스크롤 가능"
              tabIndex={0}
            >
            ...
            <EmptyState
              ...
              description="질문의 기간이나 조건을 바꿔 다시 검색해보세요."
            />
          )}
