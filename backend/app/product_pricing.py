...
def trusted_public_price(
    product: Product, today: date | None = None
) -> tuple[float | None, str, str | None, date | None]:
    business_price = trusted_business_price(product, today)
    if business_price[0] is not None:
    ...
    if (
        ...
        return price, f"공식몰 참고가 {price:,.0f}원", product.price_source_url, verified_at
    return business_price
